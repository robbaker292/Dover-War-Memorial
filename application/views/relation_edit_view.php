...
$(document).on("click", ".btn-delete", function(e) {
	bootbox.confirm({ 
		...
		callback: function(result){ 
			if(result) {
				window.location.href = <?php echo "\"".base_url()."relation/delete/".$relation->id."\""; ?>;
			}
		}
	});
});
	...
	if ($new) {
		echo "<h2>New Relation Type</h2>";
	} else {		
		echo "<h2>Editing Relation Type";
		?>
		<a href="#" class="btn btn-danger btn-md pull-right btn-delete" role="button">
			<i class="fa fa-trash-o" aria-hidden="true"></i>&nbsp;&nbsp;Delete Relation Type
		</a>
		</h2>
		<?php
	}	
...
<div id="editing">
	...
	<form id="basicForm">
		<input type="hidden" id="id" name="id" <?php if(!$new) { echo "value=\"".$relation->id."\""; }?>>

		<div class="form-group" id="titleGroup">
			<label class="control-label col-sm-3" for="name">Name:</label>
			<div class="col-sm-9">
				<input type="text" class="form-control" id="name" placeholder="Enter Name" name="name" <?php if(!$new) { echo "value=\"".$relation->name."\""; }?> >
			</div>
		</div>

		<div class="form-group">
			<div class="btn-group" role="group" aria-label="...">
				<button type="button" class="btn btn-primary" id="saveBasic">Save relation</button>
			</div>
			<div id="saveResult">
				<?php
